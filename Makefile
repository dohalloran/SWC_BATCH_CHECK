# This Makefile is for the SWC_BATCH_CHECK extension to perl.
#
...
#     NAME => q[SWC_BATCH_CHECK]
#     PL_FILES => {  }
#     PREREQ_PM => { File::Basename=>q[0], List::Util=>q[0], Modern::Perl=>q[1], Moose=>q[2], Sys::Hostname=>q[0], Term::ANSIColor=>q[0], Test::More=>q[0], Win32::Console::ANSI=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/SWC_BATCH_CHECK.pm]
#     clean => { FILES=>q[SWC_BATCH_CHECK-*] }
...
PERL_ARCHLIBDEP = /System/Library/Perl/5.18/darwin-thread-multi-2level
LIBPERL_A = libperl.a
...
PERL_INCDEP = /System/Library/Perl/5.18/darwin-thread-multi-2level/CORE
PERL = "/usr/bin/perl"
...
MAKEMAKER   = /Library/Perl/5.18/ExtUtils/MakeMaker.pm
MM_VERSION  = 7.34
MM_REVISION = 73400
...
MAKE = make
FULLEXT = SWC_BATCH_CHECK
BASEEXT = SWC_BATCH_CHECK
PARENT_NAME = 
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/SWC_BATCH_CHECK.pm
...
DISTNAME = SWC_BATCH_CHECK
DISTVNAME = SWC_BATCH_CHECK-1.0
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/SWC_BATCH_CHECK.pm $(INST_MAN3DIR)/SWC_BATCH_CHECK.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''Damien OHalloran <damienoh@gwu.edu>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: SWC_BATCH_CHECK' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "gpl_2"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	$(NOECHO) $(ECHO) '   "meta-spec" : {' >> META_new.json
	$(NOECHO) $(ECHO) '      "url" : "http://search.cpan.org/perldoc?CPAN::Meta::Spec",' >> META_new.json
	$(NOECHO) $(ECHO) '      "version" : 2' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "SWC_BATCH_CHECK",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "Modern::Perl" : "1",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Moose" : "2",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Sys::Hostname" : "0",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Term::ANSIColor" : "0",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Win32::Console::ANSI" : "0",' >> META_new.json
...
MAP_TARGET    = perl
FULLPERL      = "/usr/bin/perl"
MAP_PERLINC   = "-Iblib/arch" "-Iblib/lib" "-I/System/Library/Perl/5.18/darwin-thread-multi-2level" "-I/System/Library/Perl/5.18"
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="SWC_BATCH_CHECK" VERSION="1.0">' > SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Damien OHalloran &lt;damienoh@gwu.edu&gt;</AUTHOR>' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Basename" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="List::Util" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Modern::Perl" VERSION="1" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Moose::" VERSION="2" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Sys::Hostname" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Term::ANSIColor" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Win32::Console::ANSI" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="darwin-thread-multi-2level-5.18" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> SWC_BATCH_CHECK.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> SWC_BATCH_CHECK.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  ...
	  'lib/SWC_BATCH_CHECK.pm' 'blib/lib/SWC_BATCH_CHECK.pm' 
